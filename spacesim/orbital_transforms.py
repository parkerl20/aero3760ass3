...
def NED_to_ECEF_mtx(
    ...
    lng_r = np.radians(longitude)
    
    M11 = -np.sin(lat_r) * np.cos(lng_r)
    M12 = -np.sin(lng_r)
    M13 = -np.cos(lat_r) * np.cos(lng_r)
    
    M21 = -np.sin(lat_r) * np.sin(lng_r)
    M22 = np.cos(lng_r)
    M23 = -np.cos(lat_r) * np.sin(lng_r)
...
if __name__ == "__main__":
    location = (45.532, -72.782, 1.699)
    ecef = np.array([[1345.660, -4350.891, 4452.314]]).T
    
    print(NED_to_ECEF_mtx(*location[:2]) @ ecef)